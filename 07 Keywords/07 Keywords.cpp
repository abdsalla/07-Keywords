// 07 Keywords.cpp : Defines the entry point for the console application.
//
...
int main()
{
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		int choice = (rand() % choice - 1);
		string theWord = correct_guesses[choice];


		while (guess != "q" && winCount != 3)
		{
			...
			if (guess != theWord && guess != "q" && guess != "h")
			{
				...
				tries -= 1;
				cout << name << " you have " << tries << " left." << endl;	
			}
			// win condition below
			if (guess == theWord)
			{
				
				correct_guesses.erase[choice];
				int choice = (rand() % choice - 1);
				string theWord = correct_guesses[choice];
				tries -= 1;
				winCount += 1;
